...
async function uploadPastedImage(file) {
    try {
        ...
        if (data.success) {
            ...
            const allOrdersInfo = data.allOrderNumbers && data.allOrderNumbers.length > 1 
                ? ` (Found ${data.allOrderNumbers.length} order numbers: ${data.allOrderNumbers.join(', ')})` 
                : '';
            addMessage(`✅ Image uploaded successfully${orderInfo}${allOrdersInfo}. The support team has been notified.`, false);
        } else {
...
if (attachBtn && receiptInput) {
    ...
    receiptInput.addEventListener('change', async (e) => {
        ...
        if (file.size > 10 * 1024 * 1024) {
            alert('❌ File size must be less than 10MB. Your file: ' + (file.size / 1024 / 1024).toFixed(2) + 'MB');
            receiptInput.value = '';
            return;
        }
        ...
        try {
            ...
            addMessage(`📎 Uploading receipt: ${fileName}...`, false);
            ...
            if (response.ok && data.success) {
                ...
                addMessage(`✅ Receipt uploaded successfully${orderInfo}! Support team has been notified.`, false);
